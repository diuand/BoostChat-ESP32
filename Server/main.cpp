...
void read_from_client(Client_data client) {
    ...
    while (true) {
        ...
        if (!ec) {
            ...
            // Broadcast disconnection message to all clients except the disconnected one
            std::string disconnect_message = "Client " + boost::lexical_cast<std::string>(client.id) + " disconnected";
            for (auto& other_client : clients) {
                if (other_client.socket != client.socket) {
                    boost::asio::write(*other_client.socket, boost::asio::buffer(disconnect_message));
                }
            }
...
void handle_client(std::shared_ptr<ip::tcp::socket> socket) {
    ...
    // Broadcast disconnection message to all clients except the disconnected one
    std::string disconnect_message = "Client " + boost::lexical_cast<std::string>(id) + " connected";
    for (auto& other_client : clients) {
        if (other_client.socket != socket) {
            boost::asio::write(*other_client.socket, boost::asio::buffer(disconnect_message));
        }
    }
